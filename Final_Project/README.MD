...
This repository contains an Ethereum DApp that demonstrates a Donation tracking between a Doner and Non-profit organizations. The user story starts with the organization by registering to the platform. When it gets approval, the organization can raise a campaign. Each organization must provide how many executors involved on the campaign operations plus their information. Each Executor information must be inserted in order based on level of the operation. Depending on the campaign phase, the executor can change the operation states. if the state is completed, automatically the operation phase will move to the following executor. Every doner support those campaign can track these campaign operations and get every update of its state.
